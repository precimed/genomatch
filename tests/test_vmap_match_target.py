...
def test_match_vmap_to_target_skips_missing_provenance_duplicate_when_later_match_exists(tmp_path):
    ...
    write_lines(
        source,
        [
            "1\t100\tmissing_first\tT\tA\t.\t-1\tmissing",
            "1\t100\tmapped_second\tA\tT\tshard2\t5\tidentity",
        ],
    )
    write_lines(target, ["1\t100\tt1\tA\tT"])
    write_json(source.with_name(source.name + ".meta.json"), {"object_type": "variant_map", "target": {"genome_build": "GRCh37", "contig_naming": "ncbi"}})
    write_json(target.with_name(target.name + ".meta.json"), {"object_type": "variant_table", "genome_build": "GRCh37", "contig_naming": "ncbi"})
    ...
    assert result.returncode == 0, result.stderr
    assert read_tsv(out) == [["1", "100", "t1", "A", "T", "shard2", "5", "identity"]]
