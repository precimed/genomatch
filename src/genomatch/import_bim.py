...
def main() -> int:
    ...
    for shard in resolve_import_input_paths(args.input, kind_label=".bim", explicit_shards_csv=args.shards):
        source_index = 0
        with open(shard.path, "r", encoding="utf-8", newline="\n") as handle:
            for line in handle:
                ...
                if not is_canonical_import_allele(a1) or not is_canonical_import_allele(a2):
                    ...
                    source_index += 1
                    continue
                if len(a1) > args.max_allele_length or len(a2) > args.max_allele_length:
