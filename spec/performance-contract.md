...
2. Ownership by stage is explicit.
...
3. Redundant `PN`/`PV` is disallowed without boundary change.
...
4. Boundary checks remain strict.
...
2. Representation changed in a way that invalidates prior guarantees.
3. Boundary-local error semantics require a local check.
...
2. Justification for retained Python-native loops in hot paths.
...
3. Test evidence for unchanged behavior semantics (targeted tests at minimum; broader suite when warranted).

4. Profiling evidence when a change is motivated by performance.
...
2. Multi-status assignment must use explicit precedence:
- assign statuses in ordered precedence with guards that prevent later overwrite unless intended.

3. Stage schema contracts must be explicit:
- major stage boundaries must define required columns and validate them before use.

4. Temporary helper columns should not leak across stage boundaries unless part of the stage contract.
